...
function setValue(target: any, fieldPath: string, value: any)
{
    ...
    let path: string[] = _.toPath(fieldPath);
    if (path.length > 1) {

        let objectToBeUpdated = _.get(target, path.slice(0, path.length - 1).join('.'));
        if (objectToBeUpdated instanceof Map) {
            objectToBeUpdated.set(path[path.length - 1], value);
        } else {
            _.set(target, fieldPath, value);
        }
    } else {
        _.set(target, fieldPath, value);
    }
