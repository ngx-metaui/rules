...
export class DateAndTimeComponent extends BaseFormComponent
{
    ...
    /**
     * Defines symbolic name that is used when formatting a date. Each symbolic name is defined in
     * the localized files in order to retrieve locale aware format pattern.
     *
     * if you use both formatName and formatPattern, formatName takes a precedence and default
     * value inside formatPattern is ignored
     */
    @Input()
    formatName: string;
    ...
    /**
     * Default time format pattern used if none is passed
     *
     */
    @Input()
    formatPatternTime: string = 'hh:mm';
    ...
    private initTranslations()
    {
        ...
        this.formatPatternNG = this.formatPattern;
        this.formatNameWithTime = this.formatPattern;
        if (isPresent(this.formatter)) {
            this.formatName = (<DateFormatter> this.formatter).format;
        }

        if (isPresent(this.formatName)) {
            this.formatPattern = (<DateFormatter>this.formatters.get(this.formatName)).pattern;
            this.formatPatternNG = (<DateFormatter>this.formatters.get(this.formatName)).patternNG;

            this.showTime = this.showTime || this.formatName.toLowerCase().indexOf('time') > 0;
        ...
        this.i18n = {
            firstDayOfWeek: this.formatters.i18nService.instant('Widgets.date.firstDayOfWeek'),
            dayNames: this.formatters.i18nService.instant('Widgets.date.weekdays'),
            dayNamesShort: this.formatters.i18nService.instant('Widgets.date.weekdaysShort'),
            dayNamesMin: this.formatters.i18nService.instant('Widgets.date.weekdaysMin'),
            monthNames: this.formatters.i18nService.instant('Widgets.date.months'),
            monthNamesShort: this.formatters.i18nService.instant('Widgets.date.monthsShort')
        };
