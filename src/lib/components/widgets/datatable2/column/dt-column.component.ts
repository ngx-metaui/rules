...
export class DTColumn2Component extends BaseComponent implements AfterContentInit
{
    ...
    constructor (public env: Environment,
                 ...
                 public dt: Datatable2Component, public element?: ElementRef)
    {
        super(env);
    }
    ...
    handleHeaderClick (event: any, element: any): void
    {
        if (this.isHeaderSelectable()) {
            this.dt.headerSelectionChange(element, this);
    ...
    {
        if (isPresent(this.dt.rowSelectable)) {
            return this.dt.rowSelectable(item);
        }
    ...
    postInitialize (myIndex: number): void
    {
        ...
        if (this.dt.hasFrozenColumns()) {
            table = this.dt.el.nativeElement.querySelector('.dt-body-frozen table');
        } else {
