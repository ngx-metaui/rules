...
/* tslint:enable */
@Component({
  templateUrl: 'meta-dashboard-layout.component.html',
  styleUrls: ['meta-dashboard-layout.component.scss']
})
export class MetaDashboardLayoutComponent extends MetaLayout {
  ...
  static ZoneToc = 'zToc';
  static ZonesTB = [
    MetaDashboardLayoutComponent.ZoneToc, ZoneTop, ZoneBottom
  ];
  ...
  topLayouts(): ItemProperties[] {
    const tops = this.layoutsByZones.get(ZoneTop);
  ...
  portletWidth(name: string): any {
    ...
    return (width) && (PortletSizes[width]) ? PortletSizes[width] :
      'ui-md-4';
  }

  mainLayouts(): ItemProperties[] {
    const bottom = this.layoutsByZones.get('zMain');

    return (bottom) ? bottom : [];
  }

  bottomLayouts(): ItemProperties[] {
    const bottom = this.layoutsByZones.get(ZoneBottom);
