 ...
 *
 */
import {assert, isBlank, isPresent, isString} from '../../../core/utils/lang';
import {ListWrapper, MapWrapper} from '../../../core/utils/collection';
import {Entity, isEntity} from '../../../core/domain/domain-model';
...
export class DT2DataSource extends DataSource {
  ...
  init(...args: any[]): void {
    ...
    const init: DTDSInitParams = args[0];
  ...
  fetch(withParams?: Datatable2State): void {
    ...
    this.dataProvider.fetch(params).subscribe((result: any[]) => {
      if (withParams.offset > 0) {
        const incrData = [...this.dataProvider.dataChanges.getValue(), ...result];
  ...
  reorderRows(origPos: number, newPos: number, dropPos: DropPosition): void {
    const array = this.dataProvider.data().slice();
...
export class Datatable2State {
  ...
  static fromJSON(data: string): Datatable2State {
    const state: DTStateSerializableHelper = JSON.parse(data);
    const ds = new Datatable2State();
  ...
  static toJSON(data: Datatable2State): string {
    const toConvert: DTStateSerializableHelper = {
      offset: data.offset,
      limit: data.limit,
...
export class DetailRowExpansionState {
  ...
  isExpanded(item: any): boolean {
    ...
    const isOutlineExpanded = this.dt.isOutline() ? this.dt.outlineState.isExpanded(key) : true;
