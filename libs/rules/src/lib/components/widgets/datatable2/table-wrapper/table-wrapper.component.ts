...
export class DTWrapper extends BaseComponent implements AfterViewInit, AfterViewChecked {
  ...
  alignTablesHeights(frozenView: any, unFrozenView: any): void {
    ...
    const frozenRows: any[] = frozenView.querySelectorAll('table tr');
    const unFrozenRows: any[] = unFrozenView.querySelectorAll('table tr');
    ...
    Array.from(frozenRows).forEach((frozen: any, index: number) => {
      const h = Math.max(frozen.offsetHeight, unFrozenRows[index].offsetHeight);
  ...
  ngAfterViewChecked(): void {
    ...
    if (this.dt.hasFrozenColumns()) {
      const frozenView = this.thisElement.nativeElement.querySelector('.dt-body-frozen');
      const unFrozenView = this.thisElement.nativeElement.querySelector('.dt-body-unfrozen');

      const frozenWidth = this.calculateFrozenWidth();
  ...
  onLazyLoad(event: any): void {
    if (event.isLoad) {
      ...
      const dataProvider = this.dt.dataSource.dataProvider;
      const data = dataProvider.dataChanges.getValue();
  ...
  private calculateLimit(): number {
    const browserH = this.domUtils.browserDimentions().height;
    const rowH = this.dt.el.nativeElement.querySelector('tbody tr:first-child').offsetHeight;
