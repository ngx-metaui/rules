...
import {assert, BooleanWrapper, equals, isBlank, isPresent} from '../../../core/utils/lang';
...
export class Datatable2Component extends BaseComponent implements AWDataTable, AfterViewChecked,
  ...
  initDatasource(initialize: boolean = true): void {
    ...
    if (initialize) {
      ...
      const qType = (this.isOutline() && this.outlineFormat === 'tree') ?
  ...
  onCellSelectionChange(cell: any, column: DTColumn2Component, item: any): void {
    ...
    if (isPresent(this.state.selection) && this.state.selection.length > 0) {
      ...
      const foundIndex = ListWrapper.findIndexComplex(this.state.selection, lookupKey);
  ...
  onRowToggle(event: any, item: any): void {
    ...
    if (isPresent(this.state.selection) && this.state.selection.length > 0) {
      const foundIndex = ListWrapper.findIndexComplex(this.state.selection, item);
      const isSelected = foundIndex !== -1;
  ...
  onHandleOutlineRowToggleToChildren(currentItem: any, isSelected: boolean): void {
    const childrenForNode = this.children.apply(this.context, [currentItem]) || [];
    ...
    if (childrenForNode.length > 0) {
      ...
      if (!isSelected) {
        ...
        for (const child of childrenForNode) {
          const foundIndex = ListWrapper.findIndexComplex(this.state.selection, child);
          this.state.selection = this.state.selection
            .filter((val: any, index: number) => index !== foundIndex);
        }
      }
  ...
  oHandleOutlineRowToggleToParent(currentItem: any, isSelected: boolean): void {
    ...
    if (isPresent(parent)) {
      const childrenForNode = this.children.apply(this.context, [parent]) || [];
      ...
      if (!isSelected) {
        ...
        if (!allSelected) {
          const parentIndex = ListWrapper.findIndexComplex(this.state.selection,
            parent);
          this.state.selection = this.state.selection
  ...
  onDnDColumnDrop(origPos: number, newPos: number, dropPos: DropPosition): void {
    const source = this.env.getValue('dragSource');
    const target = this.env.getValue('dragTarget');
  ...
  doHandleDndRowXRossTable(origPos: number, newPos: number, dropPos: DropPosition): void {
    ...
    if (source === 'left' && target === 'right') {
      ...
      const removedColumnLeft = this.frozenColumns.splice(origPos, 1);
      ...
      const removedColumnLeft = this.columns.splice(origPos, 1);
  ...
  private doMoveColumns(list: DTColumn2Component[], newPos: number, origPos: number,
    ...
    const array: Array<any> = Array.from(list).slice();
  ...
  isHeaderSelected(item: DTColumn2Component): boolean {
    ...
    const colMatched = item.key || item.label;
    const currentCol = this.state.headerSelection.key || this.state.headerSelection.label;
  ...
  toggleAllColumns(event: any): void {
    const currentItems = this.dataToRender || [];
    const selectedObject = this.state.selection || [];
  ...
  private initFrozenColumns(): void {
    ...
    if (this.frozenColumns.length > 0) {
      ...
      const lastInx = this.columns.slice()
        .reverse()
        .findIndex((col: DTColumn2Component) => this.isInternalColumn(col));

      if (lastInx !== -1) {
        const idx = this.columns.length - 1 - lastInx;
        const internalCols = this.columns.splice(0, idx + 1);
        this.frozenColumns = [...internalCols, ...this.frozenColumns];

      }

      const hasValidCols = this.columns
