...
import {DragEvents, DragRowDirection, DropPosition} from '../aw-datatable';
...
export class DTDraggableRowDirective implements OnInit, OnDestroy {
  ...
  private onMouseDownEvent(event: any): void {
    if (event.altKey && this.domUtils.hasParent(event.target, '.dt-row-draggable')) {
      ...
      const elToBeDragged = this.domUtils.elementDimensions(event.target);
  ...
  private onDropEvent(event: any): void {
    ...
    const dropPos: DropPosition = this.inMiddle ? DropPosition.Into : (
  ...
  private onDragLeaveEvent(event: any): void {
    const tr = this.domUtils.closest(event.target, 'tr');
  ...
  private markRowWithClass(event: any, activeRow: any): void {
    ...
    const currentTrCenter = this.domUtils.elementDimensions(activeRow).height / 2;
    const draggedTrCenter = event.offsetY + this.dt.env.getValue('ddClickDeviance');
  ...
  private isValidDropTarget(event: any): boolean {
    ...
    const siblingRow = this.dndRowIndex - origInx;
